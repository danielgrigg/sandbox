...
#ifndef shadowmap_mesh_math_h
#define shadowmap_mesh_math_h
...
namespace lap
{
  ...
  public:
    ...
    {
      return std::equal(v, v+N, rhs.v, fequals<T>);
    }
  ...
  std::ostream& operator<<(std::ostream& os, const vec<T, N>& rhs)
  {
    ...
    void str(std::ostream& os, const vec<V,N>& rhs,
    ...
    template <typename I>
    Range<I> allocate_range(std::vector<uint32_t>& dest, size_t length)
    {
    ...
    public:
      ...
      BoundingBox(const P *points, int numPoints)
      {
        ...
        std::for_each(points, points + numPoints,
                      boost::lambda::bind(&BoundingBox::union_point, this, boost::lambda::_1));
      }
