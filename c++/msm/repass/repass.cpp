...
#define GUARD(name, predicate) struct name {\
  ...
  GUARD(dragging_in, evt._reach < fsm._last_drag_length);
  GUARD(found_payout, evt._reach > fsm._min_drag_length + FSM::min_delta_payout);
  GUARD(paying_out_max, evt._reach > fsm._max_drag_length);
  GUARD(found_repass, evt._reach < fsm._max_drag_length - FSM::min_delta_repass);
