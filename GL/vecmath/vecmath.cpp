...
bool initShaders()
{
  ...
  sources.push_back("vecmath.fp");
  sources.push_back("vecmath.vp");
...
void drawGeometry(float t)
{
  ...
  Matrix4 M2 = Matrix4::translation(Vector3(0, 0.4, -8)) * 
    Matrix4::rotationX(LRadians(30)) * Matrix4::rotationY(LRadians(7.0*t));

  float fAspect = (float)gViewWidth / (float)gViewHeight;
  Matrix4 MVP2 = Matrix4::perspective(LRadians(40.0f), fAspect, 1.0f, 1000.0f) * M2;

  glUniformMatrix4fv(uniforms[kModelView], 1, GL_FALSE, (float*)&M2);
  glUniformMatrix4fv(uniforms[kMVP], 1, GL_FALSE, (float*)&MVP2);
