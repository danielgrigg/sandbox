...
def generatePackages(platform, targetName)
  ...
  platform[:packages].map {|p| 
    ...
    result << "include_directories(${#{p[:name].upcase}_INCLUDE_DIRS})\n"
    result << "target_link_libraries(#{targetName} ${#{p[:name].upcase}_LIBRARIES})"
  }
...
def generateSources(sourceRoot)
  sources = ["set(SOURCES)"]
  sources.concat findFiles(sourceRoot, %r{\.(cpp|h|hpp)$}).map {|s|
...
def generateCMake(project, platform, scheme)
  ...
  contents << "set(CMAKE_MODULE_PATH ${CMAKE_MODULE_PATH} \"${CMAKE_SOURCE_DIR}/cmake/Modules/\")"
