...
struct GreyCanvas
{
  ...
  void save()
  {
    ...
    if (g_lcd)
    {
      ...
      gltWriteTGA("kerning.tga", w, h, outPixels);
    }
    else
    {
      ...
      gltWriteTGA("kerning.tga", m_width, m_height, outPixels);
    }
  }
};
...
int main(int argc, char **argv)
{
  ...
  pen.x = 200 * 64;
  pen.y = (512 - 200) * 64;
  unsigned long previousGlyph = 0;
  bool doKerning = true;
  for (int i = 0; charCodes[i] != 0; ++i) 
  { 
    std::cout << "'" << charCodes[i] << "'\n";

    unsigned long glyphIndex = FT_Get_Char_Index(face, charCodes[i]);

    if (doKerning && previousGlyph > 0 && glyphIndex > 0)
    {
      FT_Vector kerning;
      FT_Get_Kerning(face, previousGlyph, glyphIndex, 
          FT_KERNING_DEFAULT, &kerning);
      std::cout << "kerning " << charCodes[i-1] << " -> " << charCodes[i] <<
        ": (" << kerning.x << ", " << kerning.y << ")\n";
      pen.x += kerning.x;
    }
    ...
    uint32_t error = FT_Load_Glyph(face, glyphIndex, FT_LOAD_RENDER);
    if (face->glyph->format != FT_GLYPH_FORMAT_BITMAP)
    {
